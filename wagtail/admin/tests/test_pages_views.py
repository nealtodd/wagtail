...
class TestPageExplorer(TestCase, WagtailTestUtils):
    ...
    def test_ordering_less_than_100_pages_uses_specific_page_with_custom_display_title(
        self
    ):
        ...
        response = self.client.get(
            reverse("wagtailadmin_explore", args=(self.root_page.id,)) + "?ordering=ord"
        )
        self.assertContains(response, "New event 0 (single event)")

    def test_ordering_100_or_more_pages_uses_generic_page_without_custom_display_title(
        self
    ):
...
class TestBreadcrumb(TestCase, WagtailTestUtils):
    ...
    def test_breadcrumb_uses_specific_titles(self):
        ...
        self.assertContains(
            response,
            """<li><a href="%s">Secret plans (simple page)</a></li>""" % expected_url,
        )
...
class TestExplorablePageVisibility(TestCase, WagtailTestUtils):
    ...
    def test_admin_sees_breadcrumbs_up_to_root_page(self):
        ...
        self.assertInHTML(
            """<li><a href="/admin/pages/4/">Welcome to example.com!</a></li>""",
            str(response.content),
        )
        self.assertInHTML(
            """<li><a href="/admin/pages/5/">Content</a></li>""", str(response.content)
        )
...
class TestPageCreation(TestCase, WagtailTestUtils):
    ...
    def test_create_simplepage_post_submit(self):
        ...
        self.assertEqual(
            ...
            'The page "New page!" has been submitted for moderation',
        )
...
class TestPageEdit(TestCase, WagtailTestUtils):
    ...
    def test_page_edit_post_submit(self):
        ...
        self.assertEqual(
            mail.outbox[0].subject,
            'The page "Hello world!" has been submitted for moderation',
    ...
    def test_preview_uses_correct_site(self):
        ...
        self.assertTemplateUsed(response, "tests/simple_page.html")
        self.assertEqual(
            response.context["request"].site.hostname, "childpage.example.com"
        )
    ...
    def test_editor_page_shows_live_url_in_status_when_draft_edits_exist(self):
        ...
        self.child_page.slug = (
            "revised-slug-in-draft-only"
        )  # live version contains 'hello-world'
        self.child_page.save_revision()
...
class TestPageMove(TestCase, WagtailTestUtils):
    ...
    def test_page_move(self):
        response = self.client.get(
            reverse("wagtailadmin_pages:move", args=(self.test_page.id,))
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_page_move_confirm(self):
        response = self.client.get(
            reverse(
                ...
                args=(self.test_page.id, self.section_b.id),
            )
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_page_set_page_position(self):
        response = self.client.get(
            reverse("wagtailadmin_pages:set_page_position", args=(self.test_page.id,))
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_before_move_page_hook_post(self):
        ...
        self.assertEqual(
            Page.objects.get(id=self.test_page.id).get_parent().id, self.section_a.id
        )
    ...
    def test_after_move_page_hook(self):
        ...
        self.assertEqual(
            Page.objects.get(id=self.test_page.id).get_parent().id, self.section_b.id
        )
...
class TestContentTypeUse(TestCase, WagtailTestUtils):
    ...
    def test_content_type_use(self):
        ...
        response = self.client.get(
            reverse("wagtailadmin_pages:type_use", args=("tests", "eventpage"))
        )
...
class TestRevisions(TestCase, WagtailTestUtils):
    ...
    def test_revert_revision(self):
        ...
        self.assertContains(
            response, "You are viewing a previous revision of this page"
        )
...
class TestRevisionsUnscheduleForUnpublishedPages(TestCase, WagtailTestUtils):
    ...
    def test_unschedule_view_post(self):
        ...
        self.assertRedirects(
            response,
            reverse(
                "wagtailadmin_pages:revisions_index", args=(self.unpublished_event.id,)
            ),
        )
...
class TestInlineStreamField(TestCase, WagtailTestUtils):
    ...
    def test_inline_streamfield(self):
        ...
        self.assertContains(
            response, '<li id="__PREFIX__-container" class="sequence-member">'
        )
...
class TestRecentEditsPanel(TestCase, WagtailTestUtils):
    def setUp(self):
        ...
        get_user_model().objects.create_superuser(
            username="alice", email="alice@email.com", password="password"
        )
        get_user_model().objects.create_superuser(
    ...
    def test_panel(self):
        ...
        self.assertEqual(
            panel.last_edits[0][0].page, Page.objects.get(pk=self.child_page.id)
        )
        # check if the page in this list is the specific page of this revision
        self.assertEqual(
            panel.last_edits[0][1], Page.objects.get(pk=self.child_page.id).specific
        )
